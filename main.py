    ...
    while True:
        if waiting =="true":
            if weekday=="Sat" or weekday=="Sun":
                    choice1=g.indexbox("今天是"+weekdayDisplay+",如果进行作业复制将会自动将旧作业移动到older_versions文件夹",title="复制作业副本ver5.0:防误触",choices=("无视,继续生成","生成,但是不移动旧作业","我点错了,退出"))
                    print(choice1)
                    if choice1==0:
                        iffilemove='true'
                        waiting="false"
    ...
    for dataname in datanames:
        ...
        #print(listFile)
            if listFile == (listDir+str(datetime)+".docx"): #要求名字是今日作业母本
                copyfile='false'
                title = g.ccbox("已经存在今日作业母本(错误代码:ERROR#114514)",title="复制作业副本ver5.0:复制失败",choices=("好的","回档之前作业"))
                if title == True:
                    exit(0)
                if title == False:
                    ...
                    sourceDirtwo=sourceDirtwo+back
                    print(sourceDirtwo)
                    shutil.copy(sourceDirtwo,listDir)
                    choice2=g.ccbox("作者:LSY\n未经作者授权随意转载\n开源是一种美德。",image=imageDir+"successful.png",title="复制作业副本ver5.0:回档成功",choices=("好的","查看使用说明"))
                    if choice2 == True:
                        exit(0)
                    if choice2 == False:
                        choice3 = g.indexbox(msg="使用说明：双击本应用即可立刻复制作业母本并设置好今日日期\n复制完毕后你可以再次开启应用回档之前的作业清单。\n待更新内容:\n最终目标:通过Python的QQAPI来进行各个学科群作业关键字自动更新作业。",title="复制作业副本ver5.0:使用说明",choices=("好的","打开本项目的Github网址(你可能需要科学上网)"))
                        if choice3 == 0:
                            exit(0)
                        if choice3 == 1:
...
def Last(imageDir, moveFile):
    if iffilemove == 'true':
        ...
        movement = '\n'
    choice2=g.ccbox("作者:LSY\n未经作者授权随意转载\n"+movement+"开源是一种美德。",image=imageDir+"successful.png",title="复制作业副本ver5.0:复制成功",choices=("好的","查看使用说明"))
